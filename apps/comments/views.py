...
def get_wall_of_messages(level, msgs, childs, trees):
    ...
    for msg in msgs:
        response += [msg.in__dict({"level":level*3})]
        if msg.id in trees:
            response += get_tree(level+1, msg, Comment.objects.filter(node_history__startswith=msg.node_history))
        else:
            response += get_wall_of_messages(level+1, Comment.objects.filter(parent__id=msg.id).order_by('-date_and_time'),childs, trees)
    return response
...
def home(request):
    ...
    if request.method == 'GET':
        ...
        if request.GET.has_key("trees"):
            ...
            for msg in msgs:
                heirs_of_tree |= set(Comment.objects.filter(node_history__startswith=msg.node_history).values_list('id', flat=True))
...
def show_childs_or_tree(request):
    ...
    if request.method == 'GET':
        ...
        if(key=="trees"):
            context[key] = list(Comment.objects.filter(node_history__startswith=msg.node_history).values_list('id', flat=True))
            context['messages'] = get_tree(msg.node_history.count('-'), msg, Comment.objects.filter(node_history__startswith=msg.node_history))
        elif(key=="childs"):
            context['messages'] = get_wall_of_messages(msg.node_history.count('-'), Comment.objects.filter(parent=msg).order_by('-date_and_time'), set(), set())
            context[key] = id
...
def write_review(request, post_id):
    context = {}
    if request.method == 'POST' and request.is_ajax:
